		...
		<view class="margin padding-sm bg-white" style="border-radius:20rpx;">
			<view class="cu-card case no-card">
				<view class="cu-item shadow">
					<view class="image" style='padding:30rpx 30rpx;'>
						<image src="../../static/bg_img/login_banner.png" mode="widthFix"></image>
			...
			<view class="padding flex flex-direction">
				<button class="cu-btn bg-orange-1 round shadow-blur lg" open-type="getUserInfo"
					@click="login">统一认证登录</button>
			</view>
			<view class="margin padding solids" style="border-radius:20rpx;">
				<view>
					账号为学号，<text class="text-red">密码为统一身份认证密码</text>，初始密码为身份证后六位。
				</view>
					...
					uni.getSetting({
						success: res => {
							...
							if (res.authSetting['scope.userInfo']) {
								...
								this.$req("auth/login/", "post", postData, res => {
									if (res.code !== 10000) {
										if (res.data['cookie']) {
