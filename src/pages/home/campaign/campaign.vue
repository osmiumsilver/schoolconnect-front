<template>
  <view class="container">
    ...
    <view>
      <view class="search">
        <view @tap.stop.prevent="handlerAvatarTap" class="avatar">
          <image :src="userInfo.avatarUrl"></image>
        </view>
        <view :class="'search-wrapper ' + (show_search_back ? 'search-wrapper-blue' : '')">
          <view class="search-button" @tap="click_search">
...
<!--  -->
              ...
              <view class="card-list-view__item">
                <view class="content border-bottom">
                  <view class="content__left">
                    <image :src="item.campaignPic" v-if="item.campaignPic" mode="aspectFill"></image>
                  </view>
                  <view class="content__right">
                    <text class="location">{{ item.campaignLocation }}</text>
                    <text class="desc">{{ item.campaignName }}</text>
        ...
        <view class="loadingmore">
          <view v-if="!isEmpty">
            <button class="more-button" formType="submit" size="mini" @tap="loadMore">加载更多...</button>
          </view>
          <view class="no-more" v-if="isEmpty">---无更多活动---</view>
        </view>
        <tab-shadow/>
      </scroll-view>
    </view>
...
<script>
import util from '@/utils/util1.js';
...
export default {
  ...
  methods: {
    ...
    navToDetailPage(e) {
      uni.navigateTo({
        url: '/pages/home/campaign/detail/detail?detail=' + JSON.stringify(e)
      })
    },
